...
For a structurally migrated completed read event, first call
`python -m arisctl materialize-completed-read-event <run_id> <paper_id> <read_event_id>`
and pass its returned binding and `content` verbatim to the native
`paper_reader`; do not call `read-user-fulltext`, which creates a new event.
...
foundational work to the current frontier. This is a fixed cognition frame, not
a fixed history template: let the literature determine the number and duration
of stages and the topology of branches. Allow parallel branches, forks, merges,
long-term coexistence, and paradigm shifts; never create a stage from year
boundaries alone.
...
- directly observed negative or null evidence;
- conflicts among credible sources;
- executor-inferred boundaries that remain hypotheses.
...
Explain what method or mechanism established which progress under what
conditions, where it stopped working, why the research question migrated or
reframed, and which direction followed. Chronology, publication order, and
citation links alone do not establish this explanatory lineage. Create a trace
only for an evidence-supported research-problem or method-paradigm change. A
transition may be supported by several papers and need not have one landmark
paper; an empty trace is valid only when the evidence supports no material
transition. Keep paper-level links only for foundational, turning-point, and
current representative work in the Evidence Registry, and keep the main trace
at family level; a cross-family transition may omit `family`.
...
absence and author-written future work are leads, never proof that a problem is
real, important, or novel. Do not design a method in this Skill.
...
independently of the synthesizer's reasoning history. Look for:

- unexecuted pagination, year, venue, priority-tier, or saturation searches;
- candidates lacking title/abstract screening or unexplained abstract-only retention;
- in-scope reviews or high-citation backbone papers lacking full-text evidence;
- missing major branches or causal lineages;
- unsupported family boundaries or anchor assignments;
- decision-critical claims backed only by discovery-only evidence;
- contradictions that were flattened into false consensus;
- missing negative, diagnostic, replication, or benchmark evidence;
- unresolved cited-paper identities;
- claims of saturation unsupported by a follow-up cycle.

Always return an `evolution_assessment` with four explicit judgments:

- `foundation_to_frontier`: whether important evolution nodes from foundational
  work to the current frontier are covered;
- `key_nodes_and_branches`: whether important research problems, branches, and
  landmark or collectively representative evidence are covered;
- `transition_causality`: whether the evidence explains why a prior bottleneck,
  assumption, or question led to the subsequent direction;
- `explanatory_coherence`: whether the map explains why the field developed into
  its current form and where the present frontier sits in that evolution.

Each judgment has `status: PASS | GAP` and a non-empty `rationale`.
`transition_causality` also has a basis consistent with the Controller-bound
`development_trace_count`: use `DECLARED_TRACES_REVIEWED` when traces exist;
with no traces, use `NO_MATERIAL_TRANSITION_SUPPORTED` only when the evidence
supports gradual or parallel continuity without a material transition, or
`MATERIAL_TRANSITION_MISSING` when an important transition remains absent.
Return every material evolution gap in `material_evolution_gaps` and verbatim in
top-level `gaps`; any such gap requires `CONTINUE`. A material gap is one that
could change the field-evolution explanation, method boundary, key bottleneck,
or current-frontier judgment. An untidy timeline, irregular stage count,
parallel coexistence, or absence of a single landmark paper is not itself a gap.
...
exhaustive coverage. It is scientifically plausible only when:

1. every major family found so far has evidence and stable boundaries;
2. foundational, turning-point or influential, and current anchors are covered
   where the literature supports them;
3. decision-critical claims have decision-grade evidence and exact locators;
4. a follow-up cycle with targeted query refinement and backward/forward
   citation expansion adds no major branch, changes no family anchor, and does
   not alter the leading nearest-prior set;
5. unavailable routes and known blind spots are explicit.

It also requires all four evolution judgments to pass. Do not require a fixed
number of stages or a non-empty development trace; when the trace is empty, the
review must explicitly support that conclusion from the evidence.

If a missing route, inaccessible full text, unresolved identity, or open branch
could materially change the map, recommend `CONTINUE` or preserve a partial
status. “Not found” never means nobody has done it.
