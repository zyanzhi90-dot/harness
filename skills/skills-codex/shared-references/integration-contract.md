...
| research-lit discovery gateway | D1 fixed cascade, with a D2 aggregate only at fallback level 3 | `/research-lit` routes SerpApi Google Scholar -> real-browser `scholar.google.hk` when available -> deduplicated arXiv + IEEE Xplore -> `HUMAN_SEARCH_REQUIRED` when unavailable or still insufficient under existing research-lit judgments; no Scholar scraper or other automatic discovery source participates |
...
| `iteration_log.py` | Legacy; no active caller | Retained only for backward-compatible, user-owned custom loops. It must not be invoked by the canonical Controller workflow, `/research-pipeline`, or `/idea-discovery`. A future caller requires a Controller-aware design and a new contract row. |
