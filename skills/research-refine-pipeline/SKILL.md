...
Do not enter method refinement without a CERTIFIED problem and a hash-matched
`DIAGNOSIS_READY` root-cause handoff, and do not plan a large experiment suite
on top of an unstable route. Preserve separate problem, diagnosis, and method
verdicts.
...
- If the problem is not `CERTIFIED/accepted`, or is
  ...
  do not continue into method refinement or experiment planning.
...
- preserve the Certified Problem Contract and re-derive/verify Design
  Obligations after the Scientific Mainline;
...
- after dominant-only closure leaves a residual `MUST` gap, check the Field Map
  ...
  interfaces, capability-specific removal failures, targeted evidence, and one
  scientific closure.
...
- What causal link does each supporting mechanism close, through what interface?
...
- Are structural transfer, interfaces, assumptions, and causal story coherent?
